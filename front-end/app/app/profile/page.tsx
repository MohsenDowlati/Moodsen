...
import { computeStreak, formatDateLong } from '@/lib/dates';
...
export default function ProfilePage() {
  const { user, entries, clearEntries, signOut, reminderSettings, updateReminderSettings } = useAuth();
  ...
  const joinedDate = user ? formatDateLong(user.joinedAt.split('T')[0]) : '';
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden animate-fade-in-up">
        ...
        <div className="px-5 pb-5 sm:px-6">
          ...
          <div className="mt-5 grid grid-cols-3 gap-3">
            <MiniStat icon={<Flame className="h-4 w-4 text-orange-500" />} value={`${stats.streak}`} label="Day streak" />
            <MiniStat icon={<CalendarCheck className="h-4 w-4 text-teal-500" />} value={`${stats.total}`} label="Entries" />
            <MiniStat icon={<Smile className="h-4 w-4 text-amber-500" />} value={stats.topMood?.emoji ?? '—'} label="Top mood" />
          </div>
        </div>
      </Card>
          ...
          <div className="flex items-center justify-between rounded-xl p-3 hover:bg-muted/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Get a notification each day to log your mood</p>
              </div>
            </div>
            <Switch
            ...
            <div className="flex items-center justify-between rounded-xl p-3 hover:bg-muted/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">When the daily reminder appears</p>
                </div>
              </div>
              <Select
      ...
      <Card className="border-destructive/30 p-5 sm:p-6 animate-fade-in-up" style={{ animationDelay: '240ms' }}>
        ...
        <p className="mb-4 text-sm text-muted-foreground">These actions are permanent and cannot be undone.</p>
